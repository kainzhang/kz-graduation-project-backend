...
class ItemAnalysis(models.Model):
    ...
    stars_cnt = models.TextField(null=True, blank=True)  # 1-5 星评分情况，绘制柱状图
    senti_num = models.TextField(null=True, blank=True)  # 三种类型评论总数，绘制柱状图（横向）
    ...
    senti_score_map = models.TextField(null=True, blank=True)  # 情感分数分部图，柱状图
